var Loader = function(){
    var _isShow = false,
        ...
        init = function(){
            ...
            newdiv.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" id="' + _loadBgId +'" style="position:absolute;width:165px;height:130px;left:50%;top:50%;margin-left:-82px;margin-top:-75px;">' +
                 '<g>' +
                  '<path fill="#aaaaaa" d="'+ SVGPATH +'" />' +
                 '</g>' +
                '</svg>'+
                '<svg xmlns="http://www.w3.org/2000/svg" id="'+ _loadTopId +'" style="position:absolute;width:165px;height:0;left:50%;top:50%;margin-left:-82px;margin-top:-75px;">' +
                 '<g>' +
                  '<path fill="'+ _color+'" d="'+ SVGPATH +'" />' +
                 '</g>' +
                '</svg>';
        ...
        resize = function(){
            if(!_isShow || !_options.defaultAnimation) {
                return;
            }
            var per = _loaded/_total;
            if(per > 1) {
                per = 1;
            }
            var ww = window.innerWidth,
                ...
                lh = 130;
                ll = (ww - lw)*0.5,            
                lt = (window.innerHeight-lh)*0.5,
                loadTopId_ele = $$('#' + _loadTopId);
            _loadW = lw;
            _loadH = lh;
            loadTopId_ele.style.clip = 'clip:rect('+(lh - lh*per)+'px,'+ lw +'px,'+ lh +'px,0)';
            loadTopId_ele.style.webkitTransition = 'all 350ms linear';
            loadTopId_ele.style.transition = 'all 350ms linear';
        },
        hide = function(){
            _isShow = false;
            if(typeof _options.completeCallback =="function"){
                _options.completeCallback();
            }
            if(!_options.defaultAnimation){
                return;
            }
            var loadId_ele = $$('#'+ _loadId);
            loadId_ele.style.webkitTransition = 'all 300ms linear';
            loadId_ele.style.transition = 'all 300ms linear';
            loadId_ele.style.opacity = '0';
            setTimeout(function(){
                document.body.removeChild(loadId_ele);
            },300);
        },
        show = function(options){
            ...
            init();
            if(_options.defaultAnimation){
                $$('#'+ _loadId).style.backgroundColor = _options.bgColor;
                $$('#'+ _loadTopId).style.height = '130px';
                $$('#'+ _loadTopId).style.clip = 'rect(130px 165px 130px 0)';
            }            
            _isShow = true;
            _total = _options.iFileData.length;
            if(_total == 0){
                hide();
                return;
            }
            ...
            for(var i = 0; i < _total; i++) {
                loadImage(_options.iFileData[i]);
            }
            resize();
        ...
        checkLoadComplete = function(){
            ...
            if(typeof _options.customAnimation =="function"){
                _options.customAnimation(per);
            }
            if(_options.defaultAnimation){
                $$('#'+ _loadTopId).style.clip = 'rect('+(_loadH - _loadH*per)+'px,'+ _loadW +'px,'+ _loadH +'px,0)';
            }
            if(_loaded == _total) {
                setTimeout(function(){
                    hide();
